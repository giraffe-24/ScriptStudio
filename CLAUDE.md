...
## 現在の実装状況（自動更新）
- 最終更新: 2026-08-03
- 最終コミット: 2026-08-03 chore: エピソード61を63へ採番変更（フォルダ名・manifest id）

### 直近の実装内容
- コンテンツ追加: エピソード61「毎日の手間を消す買ってよかったもの6選」の企画・台本を追加し、その後フォルダ名とmanifest idをエピソード63へ採番変更
- ドキュメント整備: CLAUDE.md を整備し、自動更新されるステータス欄を追加。精度支援層（サブエージェント3体・自動リント・/精密チェック）も記載
- 機能追加: 参考動画URLの検証・メタ取得を共通化してテーマ活用へ組み込み、参考動画セクションを入力欄の先頭へ移動、AI深掘り回答のワンクリック反映と⌘Enter送信に対応、企画書の自動保存（新規エピソードは自動作成）
- 推敲フローの刷新: 推敲確認を変更カード方式に変更し、確定前に原文と消える内容を確認・修正できるようにした
- バグ修正: リネーム時に移動先がindexから消えるエイリアシングバグ、番号入力でのIME衝突、狭いペインでの削除モードUI崩れ（ヘッダー見切れ・削除ボタンの縦潰れ）、エピソード追加のupsert化を修正
- 運用面: 台本の自動保存にSupabase瞬断対策のリトライを追加、開発キャッシュ掃除用スクリプト（clean / dev:clean）を追加、メイン画面キャプチャを軽量化
- 2026-07-28にNASハブ移行時点の作業状態をWIP保全コミットとして記録
