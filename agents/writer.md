...
4. アプリや OS の手順・「ストアにあるか」を書くとき
   - [config/quality.md](../config/quality.md) の「製品・アプリの事実」を必ず読み、公式の最新説明と一致するか確認してから書く（推測で端末側の構成を捏造しない）。
```
...
（上記テンプレ末尾のマーカー付近は [config/calibration.md](../config/calibration.md) と一致させること。再執筆で本文を置き換えるときは、マーカー行より下＝利用者の貼り付けを可能な限り残す。）
